...
const Avatar = ({
  name, size = 36, onClick, avatarUrl, showUploadHint = false,
}: {
  name: string; size?: number; onClick?: () => void;
  avatarUrl?: string; showUploadHint?: boolean;
}) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: isClickable ? "0 0 0 2px transparent" : "none",
        position: "relative", overflow: "hidden",
      }}
      onMouseEnter={e => {
        if (isClickable) {
          (e.currentTarget as HTMLDivElement).style.transform = "scale(1.08)";
          (e.currentTarget as HTMLDivElement).style.boxShadow = "0 0 0 2px #c9a87c";
        }
        if (showUploadHint) {
          const overlay = (e.currentTarget as HTMLDivElement).querySelector(".avatar-overlay") as HTMLDivElement;
          if (overlay) overlay.style.opacity = "1";
        }
      }}
      onMouseLeave={e => {
        (e.currentTarget as HTMLDivElement).style.transform = "scale(1)";
        (e.currentTarget as HTMLDivElement).style.boxShadow = "0 0 0 2px transparent";
        const overlay = (e.currentTarget as HTMLDivElement).querySelector(".avatar-overlay") as HTMLDivElement;
        if (overlay) overlay.style.opacity = "0";
      }}
    >
      {avatarUrl ? (
        <img src={avatarUrl} alt="Profile" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: size / 2 }} />
      ) : (
        <span style={{ fontSize: size * 0.36, fontWeight: 700, color: "#5c3d1e", fontFamily: "'Syne', sans-serif" }}>{initials}</span>
      )}
      {/* Camera overlay shown on hover when upload hint is active */}
      {showUploadHint && (
        <div className="avatar-overlay" style={{
          ...
          {size >= 60 && <span style={{ fontSize: 9, color: "#fff", fontWeight: 700, marginTop: 3, letterSpacing: 0.3 }}>CHANGE</span>}
        </div>
      )}
    </div>
  );
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      const savedAvatar = localStorage.getItem(`bme-avatar-${savedID}`);
 ...
 const savedFiles = localStorage.getItem("bme-files");
  ...
  /**
   * Reads the selected image file, draws it onto an offscreen 200×200 canvas
   * (maintaining aspect ratio, centre-cropped), then exports as JPEG at 82%
   * quality. Typical output is 12–35 KB of base64 — well within localStorage.
   */
  const handleAvatarUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    // Guard: only allow image types
    if (!file.type.startsWith("image/")) {
      setAvatarUploadError("Please pick an image file.");
      return;
    }
    // Guard: raw file > 10 MB is likely a mistake
    if (file.size > 10 * 1024 * 1024) {
      setAvatarUploadError("Image too large (max 10 MB).");
      return;
    }
    ...
    reader.onload = (ev) => {
      ...
      const img = new Image();
      img.onload = () => {
        const TARGET = 200; // px — output square
        const canvas = document.createElement("canvas");
        canvas.width = TARGET;
        canvas.height = TARGET;
        const ctx = canvas.getContext("2d")!;

        // Centre-crop to square then scale to TARGET
        ...
        // Extra guard: compressed b64 should be < 80 KB
        const approxKB = Math.round((compressed.length * 3) / 4 / 1024);
        if (approxKB > 80) {
          setAvatarUploadError("Couldn't compress enough — try a smaller photo.");
          return;
        }
        ...
        if (studentID !== GHOST_ID) {
          try {
            localStorage.setItem(`bme-avatar-${studentID}`, compressed);
          } catch {
            setAvatarUploadError("Storage full — try clearing browser data.");
          }
        }
      };
      img.src = dataUrl;
  ...
  const handleRemoveAvatar = () => {
    setAvatarDataUrl("");
    setAvatarUploadError("");
    if (studentID !== GHOST_ID) localStorage.removeItem(`bme-avatar-${studentID}`);
  };
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ ...S.card, padding: "28px 20px 22px", textAlign: "center", position: "relative", overflow: "hidden" }}>
            ...
            {avatarDataUrl && (
              <button
                ...
                style={{
                  ...
                  cursor: "pointer", fontSize: 11, color: "#fff", fontWeight: 800,
          ...
          {/* Upload hint text */}
          <p style={{ fontSize: 11, color: "#c9b89a", margin: "8px 0 2px", fontWeight: 500 }}>
            {avatarUploadError
              ? <span style={{ color: "#ef4444" }}>{avatarUploadError}</span>
              : avatarDataUrl ? "Tap photo to change" : "Tap to upload a photo"}
          </p>

          {/* Display name (nickname or first name) */}
          <h2 style={{ fontSize: 22, fontWeight: 800, color: "#1a1208", margin: "10px 0 2px", fontFamily: "'Syne', sans-serif" }}>
            {displayName}
        ...
        <aside style={{ width: 260, flexShrink: 0, padding: "28px 0", display: "flex", flexDirection: "column", borderRight: "1px solid #ece8e0", background: "#faf8f4", position: "sticky", top: 0, height: "100vh" }}
          ...
          <div style={{ padding: "16px 20px 20px", borderTop: "1px solid #ece8e0" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 14, background: "#fff", border: "1px solid #ece8e0" }}>
              <Avatar name={studentName} size={36} avatarUrl={avatarDataUrl} onClick={() => setActiveTab("profile")} />
              <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <main style={{ flex: 1, minWidth: 0, overflowY: "auto", display: "flex", flexDirection: "column" }}>
          ...
          <div className="mobile-header" style={{ position: "sticky", top: 0, zIndex: 50, background: "#f7f3ed", borderBottom: "1px solid #ece8e0", padding: "0 16px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0 0" }}>
              ...
              <Avatar name={studentName} size={32} avatarUrl={avatarDataUrl} onClick={() => setActiveTab("profile")} />
            </div>
