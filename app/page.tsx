...
export default function Home() {
  ...
  // Safely check for existing password during login
  ...
  useEffect(() => {
    // Everything inside useEffect only runs on the client (browser)
  ...
  if (!isLoggedIn) {
    return (
      <div className={`min-h-screen flex items-center justify-center p-6 ${darkMode ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-950'} ${outfit.className}`}>
    ...
    <div className={`min-h-screen ${darkMode ? 'bg-slate-950 text-slate-200' : 'bg-slate-50 text-slate-900'} ${outfit.className} transition-colors duration-300`}>
      ...
      <main className="max-w-5xl mx-auto p-4 md:p-8 space-y-8 pb-32">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <a href="https://chat.whatsapp.com/EqsJ9zo4goBA6RFjv035Ei" target="_blank" className="p-4 bg-emerald-600 text-white rounded-2xl flex flex-col items-center gap-1 shadow-lg"><span className="text-2xl">💬</span><span className="font-bold text-xs">WhatsApp</span></a>
          <a href="https://drive.google.com/drive/folders/1QsLCU6OA8fswVkqO4A09ynnXSbk3PsWk" target="_blank" className="p-4 bg-blue-600 text-white rounded-2xl flex flex-col items-center gap-1 shadow-lg"><span className="text-2xl">📚</span><span className="font-bold text-xs">Resources</span></a>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className={`${darkMode ? 'bg-slate-900' : 'bg-white'} rounded-[32px] p-6 shadow-xl border border-slate-200 dark:border-slate-800 flex flex-col h-64`}>
            ...
            <textarea value={notes} onChange={(e) => {setNotes(e.target.value); localStorage.setItem('bme-notes', e.target.value)}} 
              placeholder="Type anything..." className="flex-1 w-full bg-transparent border-0 outline-none text-sm leading-relaxed"/>
          </div>
