      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {[
            ...
            { icon: <MessageCircle size={20} color="#22c55e" />, label: "BME assistant", sub: "Ask anything", bg: "#f0fdf4", action: () => {} },
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {(["Today", "This week"] as const).map((v) => (
            <button key={v} onClick={() => setShowWeekView(v === "This week")}
              style={{ padding: "6px 14px", borderRadius: 20, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, background: (v === "Today" && !showWeekView) || (v === "This week" && showWeekView) ? "#2d2416" : "#f0ebe3", color: (v === "Today" && !showWeekView) || (v === "This week" && showWeekView) ? "#f0ebe3" : "#8b7355" }}>
              {v}
            </button>
          ))}
        </div>
  ...
  const renderProgress = () => {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {/* Semester countdown */}
        <div style={{ ...S.card, padding: "20px 18px", textAlign: "center" }}>
          <p style={{ ...S.label, margin: "0 0 4px" }}>End of Semester</p>
          <p style={{ fontSize: 52, fontWeight: 800, color: "#1a1208", margin: "0 0 2px", lineHeight: 1 }}>
            {daysToEnd > 0 ? daysToEnd : "Done"}
          </p>
          {daysToEnd > 0 && <p style={{ fontSize: 13, color: "#a8967a", margin: 0 }}>days remaining · April 7, 2026</p>}
        </div>
      </div>
    );
  };
