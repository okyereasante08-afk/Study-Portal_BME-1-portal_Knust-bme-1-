...
import { 
  ...
  Download, Upload, Bell, CheckCircle, FileText
} from "lucide-react";

// --- YOUR REAL CLASS DATA ---
...
export default function Home() {
  ...
  const handleLogout = () => {
    ...
    setIsLoggedIn(false); setIsAdmin(false); setStudentID('');
    setPassword(''); setAdminAccessCode(''); setLoginMode('student');
  };
  ...
  const handleExportProfile = () => {
    ...
    const profileData = { studentID, studentName, attendance, attendanceMarked, notes, timestamp: new Date().toISOString() };
  ...
  const handleImportProfile = (event: any) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (profileData.studentID !== studentID) { alert('Error: This profile belongs to a different student ID!'); return; }
      ...
      } catch { alert('❌ Error: Invalid profile file!'); }
    };
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-md">
          <GlassCard className="p-8 border-white/20">
            ...
            <div className="flex gap-2 mb-6 p-1 bg-white/5 rounded-2xl">
              <button onClick={() => { setLoginMode('student'); setLoginError(''); setAdminAccessCode(''); }} className={`flex-1 py-2 rounded-xl text-xs font-bold uppercase transition ${loginMode === 'student' ? 'bg-[#00d4ff] text-[#0a0f1c]' : 'text-slate-400'}`}>Student</button>
      ...
      <header className="sticky top-0 z-40 p-4">
        <GlassCard className="max-w-5xl mx-auto px-6 py-4 flex justify-between items-center rounded-full border-white/10">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleExportProfile} className="p-2 bg-blue-500/10 text-blue-400 rounded-xl hover:bg-blue-500/20 transition-all flex items-center gap-1 px-3" title="Export Profile">
              <Download size={16} />
              <span className="text-[10px] font-bold uppercase hidden md:inline">Export</span>
            </button>
            <label className="p-2 bg-green-500/10 text-green-400 rounded-xl hover:bg-green-500/20 transition-all flex items-center gap-1 px-3 cursor-pointer" title="Import Profile">
              <Upload size={16} />
      ...
      <main className="max-w-5xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <button onClick={() => setShowUpdatesHub(true)} className="p-4 bg-purple-500/10 border border-purple-500/20 rounded-3xl flex flex-col items-center gap-2 hover:bg-purple-500/20 transition-all relative">
            ...
            {announcements.length > 0 && (
              <div className="absolute top-2 right-2 w-5 h-5 bg-red-500 rounded-full flex items-center justify-center text-[8px] font-black">{announcements.length}</div>
            )}
          </button>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <GlassCard className="p-6 flex flex-col h-64" delay={0.2}>
            ...
            <textarea 
              value={notes} 
              onChange={(e) => { setNotes(e.target.value); if (typeof window !== 'undefined') localStorage.setItem('bme-notes', e.target.value); }} 
        ...
        {showCWAModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-xl z-50 flex items-center justify-center p-4">
            <GlassCard className="w-full max-w-md p-8 relative">
                ...
                {COURSE_CREDITS.map(c => (
                  <div key={c.code} className="flex justify-between items-center bg-white/5 p-4 rounded-2xl border border-white/5">
                    ...
                    <input type="number" placeholder="0" min="0" max="100" className="w-16 p-2 rounded-xl bg-black/20 text-center font-bold text-[#00d4ff] outline-none border border-transparent focus:border-[#00d4ff]" 
                      onChange={(e) => { const value = Math.min(100, Math.max(0, parseInt(e.target.value) || 0)); setMarks({...marks, [c.code]: value.toString()}); e.target.value = value.toString(); }} />
                  </div>
                ))}
              </div>
              ...
              <button onClick={handleCalculateCWA} className="w-full py-4 bg-[#00d4ff] text-[#0a0f1c] rounded-2xl font-black text-xs uppercase tracking-widest">Calculate Score</button>
            </GlassCard>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {showUpdatesHub && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-xl z-50 flex items-center justify-center p-4">
            <GlassCard className="w-full max-w-3xl p-8 relative max-h-[90vh] overflow-y-auto">
                ...
                {announcements.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {announcements.map((ann: any) => (
                      <div key={ann.id} className={`p-4 rounded-2xl border-l-4 ${ann.type === 'urgent' ? 'border-red-500 bg-red-500/10' : ann.type === 'quiz' ? 'border-orange-500 bg-orange-500/10' : ann.type === 'deadline' ? 'border-yellow-500 bg-yellow-500/10' : 'border-blue-500 bg-blue-500/10'}`}>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {files.map((file: any) => (
                      <div key={file.id} className="p-4 rounded-2xl bg-white/5 border border-white/10">
                        ...
                        <p className="text-xs opacity-50 font-bold uppercase mb-3">{file.week}</p>
                        <a href={file.url} target="_blank" rel="noopener noreferrer" className="block w-full p-2 bg-blue-600 text-white rounded-lg text-xs font-bold text-center hover:bg-blue-700 transition">
                          {file.type === 'notes' ? '📄 Notes' : file.type === 'recording' ? '🎥 Recording' : file.type === 'slides' ? '📊 Slides' : '📖 Manual'}
                        </a>
                      </div>
                    ))}
                  </div>
              ...
              <div>
                <h3 className="text-lg font-black mb-4 text-white">📚 Course Updates</h3>
